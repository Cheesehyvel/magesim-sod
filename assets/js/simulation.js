        ...
        for (var i=0; i<this.threads; i++) {
            this.workers[i] = new SimulationWorker((result) => {
                if (!sum) {
                    ...
                    if (result.min_hps < sum.min_hps)
                        sum.min_hps = result.min_hps;
                    if (result.max_hps > sum.max_hps)
                        sum.max_hps = result.max_hps;
                    sum.avg_hps = (sum.avg_hps * sum.iterations + result.avg_hps * result.iterations) / (sum.iterations + result.iterations);
